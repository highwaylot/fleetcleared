...
9. **Waitlist:** self-hosted, no third party. In Vercel: **Storage -> Create Database -> Redis** (free tier), then connect it to this project -- that sets `KV_REST_API_URL` / `KV_REST_API_TOKEN` automatically. In **Settings -> Environment Variables**, add `ADMIN_SECRET` (any password you pick) for Production, then redeploy. Signups from the home page's "Get first pick when we open" form land in that store via `api/waitlist.py`. Read them at the private admin page linked from the status page, or directly at `/waitlist-7f2c9a1d.html` (enter your `ADMIN_SECRET` once; the page remembers it). It lists everyone with a "Copy all emails" button.
