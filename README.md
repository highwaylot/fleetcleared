...
9. **Waitlist (optional):** go to web3forms.com, enter your email, get a free access key (no account needed). In their dashboard, restrict it to `fleetcleared.com` so it can't be used from anywhere else. Paste the key into `web3forms_key` in `tools/launch.json` and rebuild; the "Get first pick when we open" form appears on the home page and every guide's call-to-action. Leave the key blank to skip it entirely -- the form just doesn't render. Signups land in your Web3Forms inbox (250 free a month); paste them to Claude to add to the business book's Waitlist tab.
